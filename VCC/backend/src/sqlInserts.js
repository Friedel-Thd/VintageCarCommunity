 ...
 */
export function createUser(db, user, res) {
    db.serialize(() => {
        const insertUserStmt = db.prepare(`
            INSERT INTO nutzer (userID, email, passwort, vorname, nachname, telefonnummer, stadt, plz, strasse, hnr) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        `);
        insertUserStmt.run(user.getuserid(), user.getEmail(), user.getPasswort(), user.getVorname(), user.getNachname(), user.getTelefonnummer(), user.getStadt(), user.getPlz(), user.getStrasse(), user.getHnr(), (err) => {
            if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
function createLagerhalter(db, userId, res) {
    const insertLagerhalterStmt = db.prepare(`
        INSERT INTO lagerhalter (userId) VALUES (?);
    `);
    insertLagerhalterStmt.run(userId, (err) => {
        if (err) {
    ...
    const getLagerhalterIDStmt = db.prepare(`
        ...
        WHERE userid = ?
    `);
    getLagerhalterIDStmt.get(userId, (err, row) => __awaiter(this, void 0, void 0, function* () {
        try {
            if (err) {
                res.sendStatus(500);
                ...
                if (row) {
                    ...
                    `);
                    createLagerStmt.run(lagerhalterID, (err) => {
                        if (err) {
                            console.error('Error creating Lager:', err);
                        }
                        else {
 ...
 * @param {express.Response} res - The Express response object
 */
function createEinlagerer(db, userId, res) {
    const insertEinlagererStmt = db.prepare(`
        INSERT INTO einlagerer (userId) VALUES (?);
    `);
    insertEinlagererStmt.run(userId, (err) => {
        if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
function createWerkstatt(db, userId, res) {
    const insertWerkstattStmt = db.prepare(`
        INSERT INTO werkstatt (userId) VALUES (?);
    `);
    insertWerkstattStmt.run(userId, (err) => {
        if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function setFahrzeugDaten(db, userData, res) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function setWerkstattinfo(db, userID, userData, res) {
    ...
    `);
    updateWerkstattInfoStmt.run(userData.data.werkstattName, userData.data.plz, userData.data.stadt, userData.data.street, userData.data.hnr, userData.data.angeboteneArbeiten, userData.data.marke, userID, (error) => {
        if (error) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function createService(db, lagerhalterID, userData, res) {
    const insertServiceStmt = db.prepare(`
        INSERT INTO zusatzservice (anbieter, bezeichnung, beschreibung, preis) 
        VALUES (?, ?, ?, ?);
    `);
    insertServiceStmt.run(lagerhalterID, userData.ServiceName, userData.ServiceBeschreibung, userData.ServicePreis, (err) => {
        if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function createAngebotsanfrage(db, einlagererID, kategorieID, lagerhalterID, res) {
    const insertAngebotsanfrageStmt = db.prepare(`
        INSERT INTO angebotsanfrage (einlagererid, lagerhalterid, kategorieid, beantwortet) 
        VALUES (?, ?, ?, ?);
    `);
    insertAngebotsanfrageStmt.run(einlagererID, lagerhalterID, kategorieID, 0, (err) => {
        if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function createFahrzeug(db, stellplatzID, einlagererid, marke, modell, res) {
    const createFahrzeugStmt = db.prepare(`
        INSERT INTO fahrzeug (stellplatzid, einlagererid, marke, modell, fahrbereit, wartungsstand)
        VALUES (?, ?, ?, ?, ?, ?);
    `);
    createFahrzeugStmt.run(stellplatzID, einlagererid, marke, modell, 1, '', (err) => {
        if (err) {
 ...
 */
export function changeVerfuegbarkeit(db, stellplatzID) {
    const updateVerfuegbarkeitStmt = db.prepare(`
        ...
        WHERE stellplatzid = ?;
    `);
    updateVerfuegbarkeitStmt.run(stellplatzID, (err) => {
        if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function createAngebot(db, lagerhalterID, einlagererID, stellplatzID, inhalt, res) {
    const insertAngebotStmt = db.prepare(`
        INSERT INTO angebot (lagerhalterid, einlagererid, stellplatzid, inhalt)
        VALUES (?, ?, ? ,?);
    `);
    insertAngebotStmt.run(lagerhalterID, einlagererID, stellplatzID, inhalt, (err) => {
        if (err) {
 ...
 * @param {express.Response} res - The Express response object
 */
export function setLagerDaten(db, userID, userData, res) {
    ...
    updateLagerDatenStmt.run(userData.data.servicezeiten, userData.data.postleitzahl, userData.data.stadt, userData.data.strasse, userData.data.hausnummer, userData.data.marke, userID, (error) => {
        if (error) {
...
//# sourceMappingURL=sqlInserts.js.map